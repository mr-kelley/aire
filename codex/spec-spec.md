...
- Role definitions (governed by `codex.role.base.md`).
...
- Specs make Codex's work auditable. The user can review what Codex intends to build before Codex builds it.
...
**Rules:**
- Codex MUST NOT begin implementation of a new module, feature, or component without a governing spec.
- If a spec doesn't exist for the target work, Codex MUST create one (or propose one for user review) before writing implementation code.
...
- The user MAY waive spec-first for trivial changes (typo fixes, formatting, one-line bug fixes). Codex should use judgment — if the change affects behavior, it needs a spec.
...
All projects MUST maintain a spec index at `specs/INDEX.md`. See `codex/documentation-spec.md` for the spec index format and maintenance rules.

The spec index helps Codex locate specs without directory traversal and gives humans an overview of what's been specified.
...
If a required behavior is missing from a spec, Codex MUST flag it and amend the spec before implementing. Guessing critical behaviors is not permitted.
...
- time: 2026-03-21
- summary: Adapted for Codex single-agent + human model. Replaced Claude Code references with Codex throughout. Preserved spec-first principles, spec-to-implementation mapping, behavioral declaration requirements, and all required sections.
