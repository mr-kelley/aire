...
# Purpose
Define a human-readable, always-current project state file shared between Codex and the user. The tracker is the authoritative snapshot of what is happening in the project — what work is active, what has been done, and what the current structure looks like. It provides continuity across Codex sessions.
...
- Providing session-start context so Codex can resume effectively.
...
# Format
The tracker MUST be Markdown. It is meant to be read by both Codex and the user at any time.
...
### 7. Session Notes
Optional. Brief notes from the most recent session — what was done, what was left in progress, anything Codex or the user should pick up next time. This section is overwritten each session, not appended.

# Maintenance Rules

- Codex MUST update the tracker when meaningful state changes occur: starting work, completing work, making decisions, encountering blockers, or changing project structure.
- Codex MUST NOT update the tracker for trivial changes (typo fixes, minor reformatting) unless they affect project state.
- The tracker MUST remain concise. If a section grows unwieldy, Codex should summarize and trim, preserving references to where full details live (git log, decision log, specs).
- The user MAY edit the tracker directly. Codex MUST treat user edits as authoritative and not silently revert them.
- On session start, Codex SHOULD read the tracker to restore context.
...
- time: 2026-03-21
- summary: Adapted for Codex single-agent + human model. Replaced Claude Code references with Codex throughout. Preserved human-readable Markdown format, required sections, maintenance rules, and human-oriented design.
