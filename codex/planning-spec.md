...
- How Codex creates, uses, and maintains planning artifacts.
...
- Role definitions (governed by `codex/codex.role.base.md`).

# Northstar

## Purpose
The northstar is the project's guiding document. It tells Codex (and the user) what this project is, why it exists, what success looks like, and what principles govern decisions when trade-offs arise. It is the first document Codex should read after STATE.md to understand the project's intent.
...
- Each criterion should be testable: Codex (or the user) should be able to look at the project and say "yes, this is met" or "no, it isn't."
...
- These directly inform Class B/C decisions — when Codex faces a choice, guiding principles help pick the right option.

### Non-Goals
- What this project deliberately does not aim to do. Helps prevent scope creep.

## Rules
- The northstar is written collaboratively by the user and Codex at project start.
- Codex MUST consult the northstar when making architectural or design decisions. If a choice conflicts with guiding principles, Codex flags it.
...
- Codex MUST update milestone status as work progresses. Moving a milestone to `completed` requires that its outcome conditions are verifiably met.
...
- **One sprint, one branch.** Each sprint maps 1:1 to a git work branch per `codex/codex.git-hygiene.md`. The sprint starts when the work branch is created; the sprint completes when the work is promoted to `main`. No mixing sprints on a branch, no splitting a sprint across branches.
- Codex MUST update sprint status as work progresses.
- Codex MUST NOT start a new sprint while the current sprint has uncommitted or unverified work. Complete or explicitly pause the current sprint first.
...
2. Codex drafts a NORTHSTAR.md for user review.
3. Codex drafts a ROADMAP.md with initial milestones for user review.
4. User and Codex refine both documents.
5. Codex creates sprint files for the first milestone.
6. Work begins on the first sprint.

## During Execution
- Codex works one sprint at a time.
...
- Planning artifacts inform decisions: when a Class B/C decision arises, Codex checks northstar guiding principles and current milestone context.
...
- time: 2026-03-21
- summary: Adapted for Codex single-agent + human model. Replaced Claude Code references with Codex throughout. Preserved all planning artifact definitions, workflow integration, and verification requirements.
