...
# Purpose
Create a searchable, auditable decision record so that Codex and the user can consult past decisions instead of relying on conversation memory (which does not survive across sessions).
...
- Decision classification and governance behavior for the Codex + human model.
...
- Git commit policy (governed by `codex.git-hygiene.md`).
...
- `made_by` (string): `"codex"`, `"user"`, or `"codex+user"`
...
**Class A — Free to decide.** Safe defaults, refactors preserving behavior, style/formatting, internal naming, test improvements. Codex proceeds without logging (though logging is permitted).

**Class B — Decide and log.** Behavior changes, public API shape, storage formats, security posture, performance tradeoffs, new dependencies, architectural patterns affecting multiple modules. Codex proceeds and records a decision event.

**Class C — Escalate to user.** Irreversible or high-risk actions: data destruction, privilege changes, licensing changes, major architecture pivots, paid services, production-impacting operations. Codex presents options and a recommendation, then waits for user decision. The decision event records the escalation and outcome.
...
**`dec list [filters]`** — Structured filtering: `--class B`, `--made-by codex`, `--since 2026-01-01`, `--project aire`.

**`dec rebuild-index`** — Delete and regenerate `decisions/index.sqlite` from canonical events.

# Governance Behavior

## When Codex MUST log a decision
...
- The user makes a significant project direction choice (Codex logs it on their behalf with `made_by: "user"` or `"codex+user"`).

Reinforcement: these triggers always require a decision event.

## Codex's decision behavior by class
...
```json
{
  ...
  "context": {
    "task_description": "Adapting Aire specs for Codex single-agent model",
    "spec_refs": ["codex/state-tracker-spec.md"],
    "assumptions": ["State is shared between Codex and human, not between AI agents"]
  },
  "scope": {
    "areas": ["governance", "state"],
    "paths": ["state/tracker.md", "codex/state-tracker-spec.md"]
  },
  "options": [
  ...
  "rationale": "The tracker is read by humans as often as by Codex. Markdown is the most natural format for both audiences and diffs cleanly in git.",
  "risk": {
    "level": "low",
    "reversibility": "high",
  ...
  "links": {
    "commit": null,
    "related_files": ["codex/state-tracker-spec.md"]
  },
  "outcome": {
    "status": "success",
    "verified_by": ["codex/state-tracker-spec.md v0.1.0"],
    "notes": null
  },
...
## Provenance
- source: Adapted from `claude/decision-log-spec.md` v0.2.0
- time: 2026-03-21
- summary: Adapted for Codex single-agent + human model. Replaced Claude references with Codex throughout. Changed made_by values from claude/claude+user to codex/codex+user. Preserved core concepts: append-only JSON events, derived SQLite index, decision classes A/B/C, CLI contract.
